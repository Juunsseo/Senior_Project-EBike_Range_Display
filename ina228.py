...
class INA228:
    ...
    def set_config(self):
        cfg = 0
        cfg |= (INA228_CONVERSION_DELAY & 0xFF) << INA228_CONVDLY_NBIT
        cfg |= (INA228_TEMP_COMP & 0x1) << INA228_TEMPCOMP_NBIT
        cfg |= (INA228_ADCRANGE & 0x1) << INA228_ADCRANGE_NBIT
        self.write_register16(self.__INA228_CONFIG, cfg)

    def set_adc_config(self):
        adc = 0
        adc |= (INA228_ADC_MODE & 0xF) << INA228_ADC_MODE_NBIT
        adc |= (INA228_VBUS_CONV_TIME & 0x7) << INA228_VBUS_CONV_TIME_NBIT
        adc |= (INA228_VSHCT_CONV_TIME & 0x7) << INA228_VSHCT_CONV_TIME_NBIT
        adc |= (INA228_VTCT_CONV_TIME & 0x7) << INA228_VTCT_CONV_TIME_NBIT
        adc |= (INA228_ADC_AVG & 0x7) << INA228_AVG_NBIT
        self.write_register16(self.__INA228_ADC_CONFIG, adc)
    
    def shunt_calib(self):
        scale = 4 if (INA228_ADCRANGE == 1) else 1
        calib_value = int(13107.2e6 * self.get_current_lsb() * INA228_SHUNT_OHMS * scale)
        if calib_value > 0xFFFF: calib_value = 0xFFFF
    ...
    def get_shunt_voltage(self):
        # Correct LSBs (datasheet): range0=312.5nV, range1=78.125nV
        if (INA228_ADCRANGE == 0):
            conversion_factor = 312.5e-9
        else:
            conversion_factor = 78.125e-9

        raw = self.read_register24(self.__INA228_VSHUNT)
        # Q20.4 format -> shift by 4
        vshunt = self.__convert2comp2float(raw >> 4, 20, conversion_factor)
        return vshunt
